#!/usr/bin/env python3
"""Fix predictions by date display with color-coded matching."""
...
old_section = '''                        # Display prediction sets
                        if sets and isinstance(sets, list):
                            ...
                            sets_cols = st.columns(min(len(sets), 3))
...
if old_section in content:
    ...
    with open('streamlit_app/pages/predictions.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print('✓ "By Date" display section updated successfully')
else:
    print('ERROR: Pattern not found in "By Date" section')
